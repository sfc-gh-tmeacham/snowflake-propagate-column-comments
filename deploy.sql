...
-- *********************************************************************************************************************
-- 5. `PROPAGATE_COMMENTS_FOR_SCHEMA` Procedure
-- This procedure iterates over all tables in a schema and calls the `RECORD_COMMENT_PROPAGATION_DATA` for each.
-- *********************************************************************************************************************
CREATE OR REPLACE PROCEDURE PROPAGATE_COMMENTS_FOR_SCHEMA(P_DATABASE_NAME VARCHAR, P_SCHEMA_NAME VARCHAR)
  COPY GRANTS
  RETURNS VARIANT
  LANGUAGE SQL
  COMMENT = 'Iterates over all tables in a schema and calls RECORD_COMMENT_PROPAGATION_DATA for each.'
  EXECUTE AS OWNER
AS
$$
DECLARE
  result_log ARRAY := ARRAY_CONSTRUCT();
  db_info_schema_fqn VARCHAR;
  tables_view_fqn VARCHAR;
  call_stmt VARCHAR;
  call_result VARCHAR;
  ...
  -- Cursor to iterate over all tables in the specified schema
  c1 CURSOR FOR 
    SELECT TABLE_NAME 
    FROM IDENTIFIER(?)
    WHERE TABLE_SCHEMA = ? 
...
BEGIN
  -- Validate inputs
  IF (P_DATABASE_NAME IS NULL OR P_SCHEMA_NAME IS NULL) THEN
    err_msg := 'ERROR in PROPAGATE_COMMENTS_FOR_SCHEMA: Input parameters cannot be NULL.';
    SYSTEM$LOG_FATAL(err_msg);
    -- Construct a failure object and return it as a VARIANT
    LET error_object := OBJECT_CONSTRUCT('status', 'FAILED', 'error', err_msg);
    RETURN ARRAY_TO_VARIANT(ARRAY_CONSTRUCT(error_object));
  END IF;

  db_info_schema_fqn := SAFE_QUOTE(P_DATABASE_NAME) || '.INFORMATION_SCHEMA';
  tables_view_fqn := db_info_schema_fqn || '.TABLES';
  
  SYSTEM$LOG_INFO('Starting schema-level propagation for ' || P_DATABASE_NAME || '.' || P_SCHEMA_NAME);

  -- Open the cursor with the fully qualified view name and schema name
  OPEN c1 USING (tables_view_fqn, P_SCHEMA_NAME);

  -- Loop through each table in the cursor
  FOR record IN c1 DO
    LET table_name := record.TABLE_NAME;
    SYSTEM$LOG_INFO('Processing table: ' || table_name);

    BEGIN
      -- Construct the dynamic CALL statement
      call_stmt := 'CALL RECORD_COMMENT_PROPAGATION_DATA(''' || P_DATABASE_NAME || ''', ''' || P_SCHEMA_NAME || ''', ''' || table_name || ''')';
      
      -- Execute the call and capture the result
      EXECUTE IMMEDIATE :call_stmt;
      SELECT "RECORD_COMMENT_PROPAGATION_DATA" INTO :call_result FROM TABLE(RESULT_SCAN(LAST_QUERY_ID()));
      
      -- Parse the RUN_ID from the return message
      run_id := SPLIT_PART(call_result, 'RUN_ID: ', 2);
      
      -- Log success and append to results
      LET success_object := OBJECT_CONSTRUCT('table_name', table_name, 'status', 'SUCCESS', 'run_id', run_id);
      result_log := ARRAY_APPEND(result_log, success_object);

    EXCEPTION
      WHEN OTHER THEN
        err_msg := 'Failed to process table ' || table_name || '. Error: ' || SQLERRM;
        SYSTEM$LOG_ERROR(err_msg);
        
        -- Log failure and append to results
        LET failure_object := OBJECT_CONSTRUCT('table_name', table_name, 'status', 'FAILED', 'error', err_msg);
        result_log := ARRAY_APPEND(result_log, failure_object);
    END;
  END FOR;

  SYSTEM$LOG_INFO('Finished schema-level propagation.');
  
  RETURN ARRAY_TO_VARIANT(result_log);
END;
$$;
